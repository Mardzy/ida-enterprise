...
const generic = {
  ...
  resolve: {
    alias: {
      ...
      "markercluster-css": path.resolve(__dirname, "node_modules/react-leaflet-markercluster/dist/styles.min.css"),
  ...
  plugins: [
    ...
    new HtmlWebpackPlugin({
      title: "Connecterra - Partner Dashboard",
 ...
 */
const wkConfig = buildTools.merge(
  {},
  ...
  /**
   * Set process.env.NODE_ENV variable to production if we build for staging or production.
   */
  buildTools.useIf(buildTools.setFreeVariable("process.env.NODE_ENV", "production"), buildTools.isBuilding()),

  /**
   * Set process.env.BUILD_ENV variable to production if we build production.
   */
  buildTools.useIf(buildTools.setFreeVariable("process.env.BUILD_ENV", "production"), buildTools.isProduction()),
